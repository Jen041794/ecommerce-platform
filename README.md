...
### 商品列表頁
- [ ] 首次載入顯示骨架卡片，資料回來後淡入真實商品
- [ ] 卡片正確顯示圖片、標題、評分、價格
- [ ] 有折扣商品顯示「-XX%」標籤、原價劃掉、紅色特價
- [ ] RWD：手機 1 欄 / 平板 2 欄 / 桌機 3~4 欄
- [ ] 邊界：庫存 0 → 按鈕顯示「已售罄」且不可點
- [ ] 邊界：API 失敗 → 顯示錯誤訊息與重新載入按鈕

### 關鍵字搜尋
- [ ] 輸入關鍵字停手約 0.5 秒後才更新列表
- [ ] 標題切換為「搜尋結果：「關鍵字」」
- [ ] 邊界：連續快速輸入只發出最後一次請求（可用 DevTools Network 驗證 Debounce）
- [ ] 邊界：查無結果顯示友善提示
- [ ] 清除（✕）回到熱門商品列表
- [ ] UI：搜尋框 focus 無外框、清除鈕四角圓角

### 商品詳情頁
- [ ] 多圖可左右滑動，有分頁圓點與左右箭頭
- [ ] 顯示品牌/分類、評分、折扣價+原價、庫存、描述
- [ ] 使用者評論顯示評論者、星等、內容、日期
- [ ] 邊界：庫存 ≤ 5 顯示「即將售完」
- [ ] 邊界：售罄 → 加入購物車按鈕 disabled
- [ ] 邊界：無評論顯示「目前還沒有評論」
- [ ] 邊界：網址輸入不存在的商品 ID → 顯示找不到並可回首頁/重載

### 購物車（防呆重點）
- [ ] 正確列出商品（圖、標題、單價、數量、小計）
- [ ] 小計 − 折扣 = 總計，改任何數量即時重算
- [ ] 邊界：數量減到 1 時「−」鈕 disabled（不可變 0）
- [ ] 邊界：數量達庫存上限時「+」鈕 disabled
- [ ] 邊界：數字框直接輸入超過庫存 → 夾回上限並跳 Toast 提示
- [ ] 邊界：輸入 0 或負數 → 自動修正為 1
- [ ] 邊界：同商品重複加入 → 數量累加，不新增重複列
- [ ] 移除商品 → 滑出動畫 + 總計重算
- [ ] 折扣碼 `WELCOME2026`（大小寫皆可）→ 總額打 9 折
- [ ] 邊界：錯誤折扣碼 → Toast「折扣碼無效」
- [ ] 空車 → 顯示空狀態與「去逛逛」按鈕

### 結帳
- [ ] 三步驟流程與步驟指示器正確高亮
- [ ] 送出後顯示訂單編號、購物車清空、徽章歸零
- [ ] 訂單寫入 LocalStorage（鍵：`ecommerce_orders`）
- [ ] 邊界：必填欄位（姓名/電話/地址）留空 → 擋下並標紅
- [ ] 邊界：電話需 09 開頭、共 10 碼；輸入時自動濾除非數字並限制 10 碼
- [ ] 邊界：Email 格式錯誤 → 擋下（留空可過，非必填）
- [ ] 邊界：信用卡卡號空白/太短 → 擋下；改貨到付款後卡號欄消失可直接過
- [ ] 邊界：空車直接進入 `/checkout` → 顯示「購物車是空的，無法結帳」

### 全域
- [ ] Navbar RWD：手機收合成漢堡並可展開
- [ ] 購物車徽章即時反映總件數
- [ ] Toast 出現在 Navbar 下方、不遮擋購物車圖示、2.5 秒自動消失
- [ ] 跨頁導航正常（react-router）
